...
export function embedFor(url: string): EmbedInfo | null {
  ...
  if (host.endsWith('instagram.com')) {
    ...
    if (!code) return null;
    return { platform: 'instagram', src: `https://www.instagram.com/p/${code}/embed`, vertical: true };
  }

  if (host.endsWith('tiktok.com')) {
    const id = TT_ID.exec(href)?.[1];
    if (!id) return null;
    return { platform: 'tiktok', src: `https://www.tiktok.com/embed/v2/${id}`, vertical: true };
  }

  if (host.endsWith('vimeo.com')) {
    const id = VIMEO_ID.exec(href)?.[1];
    if (!id) return null;
    return { platform: 'vimeo', src: `https://player.vimeo.com/video/${id}`, vertical: false };
  }
